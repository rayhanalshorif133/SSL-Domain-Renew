                                ...
                                @foreach ($domainContents as $domain)
                                    ...
                                    <pre>{{ json_encode($domain->client_email, JSON_PRETTY_PRINT) }}</pre>
                                        ...
                                        <td class="action-btns">
                                            ...
                                            <form id="deleteForm{{ $domain->id }}" class="display:inline-block;"
                                            ...
                                            {{-- <a href="{{ route('domain.sendMail', $domain->id) }}"
                                        class="btn btn-primary btn-sm">
                                        <i class="fas fa-paper-plane"></i>
                                    </a>

                                    <button type="button" class="btn btn-primary btn-sm"
                                        onclick="sendMail({{ $domain->id }})">
                                        <i class="fas fa-paper-plane"></i>
                                    </button> --}}

                                            <button type="button" class="btn btn-primary btn-sm"
                                                onclick="sendeMail({{ $domain->id }})">
                                                <i class="fas fa-paper-plane"></i>
                                            </button>
                                        </td>
                                    </tr>
                ...
                <div class="modal-body">
                    ...
                    <form id="domainForm" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input id="tagsInput" class="form-control" name="client_email">
                        </div>
...
@push('scripts')
    ...
    <script>
        ...
        function confirmDelete(id) {
            Swal.fire({
                title: 'Are you sure?',
                text: "This domain will be permanently deleted!",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    // Submit the form to delete the domain
                    document.getElementById('deleteForm' + id).submit();
                }
            });
        }

        function sendeMail(domainId) {
            Swal.fire({
                title: 'Are you sure?',
                text: "Do you want to send email to this client?",
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('/domain/send-domain-mail') }}/" + domainId,
                        type: "POST",
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
                        success: function(response) {
                            Swal.fire({
                                ...
                                title: response.status === 'success' ? 'Success' : 'Failed',
